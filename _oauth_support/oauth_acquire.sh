...
if [ -z "${AUTH_DATA:-}" ]; then
    ...
    # Check for required MCP_HANDLE_API_KEY
    if [ -z "${MCP_HANDLE_API_KEY:-}" ]; then
        echo "Error: MCP_HANDLE_API_KEY environment variable must be set for OAuth flow"
        echo "Please set your MCPHandle API key: https://www.mcp-handle.dev/home/api-keys"
        return 128
    fi
    ...
    INSTANCE_RESPONSE=$(curl --silent --request POST \
        --url https://api.mcp-handle.dev/mcp-server/self-hosted/instance/create \
        --header "Authorization: Bearer $MCP_HANDLE_API_KEY" \
    ...
    timeout 600 bash -c "
        ...
        while true; do
            AUTH_RESPONSE=\$(curl --silent --request GET \\
                --url \"https://api.mcp-handle.dev/mcp-server/instance/get-auth/$INSTANCE_ID\" \\
                --header \"Authorization: Bearer $MCP_HANDLE_API_KEY\")
