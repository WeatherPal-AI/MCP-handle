...
import { MCPHandle } from 'mcp_handle';
...
async function main() {
    const mcp_handleClient = new MCPHandle({ apiKey: process.env.MCP_HANDLE_API_KEY! });
    ...
    const response = await mcp_handleClient.mcpServer.createStrataServer({
        userId: 'demo_user',
        servers: [MCPHandle.McpServerName.Gmail, MCPHandle.McpServerName.Youtube],
    });
    ...
    const mcpTools = await mcp_handleClient.mcpServer.listTools({
        ...
        format: MCPHandle.ToolFormat.LangChain
    });
