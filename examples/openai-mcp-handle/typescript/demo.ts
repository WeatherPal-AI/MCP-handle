...
async function openaiWithMcpServer(mcpServerUrl: string, userQuery: string): Promise<string> {
    ...
    const mcpServerTools = await mcp_handleClient.mcpServer.listTools({
        ...
        format: MCPHandle.ToolFormat.Openai
    });
...
async function demoGmailIntegration() {
    ...
    const gmailMcpServer = await mcp_handleClient.mcpServer.createServerInstance({
        serverName: MCPHandle.McpServerName.Gmail,
        userId: "1234"});
    ...
    const EMAIL_RECIPIENT = "zihaolin@mcp-handle.dev"; // Replace with your email
...
async function main() {
    try {
        ...
        console.log("🛠️  You can now build powerful AI applications with OpenAI + MCPHandle MCP servers!");
