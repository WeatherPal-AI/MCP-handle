...
server = mcp_handle.mcp_server.create_server_instance("GOOGLE_DRIVE", "user123")
```
...
```bash
# Pull latest image
docker pull ghcr.io/weatherpal-ai/google-drive-mcp-server:latest


# Run Google Drive MCP Server with OAuth Support through MCP Handle AI
docker run -p 5000:5000 -e MCP_HANDLE_API_KEY=$MCP_HANDLE_API_KEY \
  ghcr.io/weatherpal-ai/google-drive-mcp-server:latest
...
**OAuth Setup:** Google Drive requires OAuth authentication. Use `MCP_HANDLE_API_KEY` from your [free API key](https://www.mcp-handle.dev/home/api-keys) to handle the OAuth flow automatically.
