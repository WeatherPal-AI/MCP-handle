...
server = mcp_handle.mcp_server.create_server_instance("MAILCHIMP", "user123")
```
...
```bash
# Pull latest image
docker pull ghcr.io/weatherpal-ai/mailchimp-mcp-server:latest


# Run Mailchimp MCP Server with OAuth Support through MCP Handle AI
docker run -p 5000:5000 -e MCP_HANDLE_API_KEY=$MCP_HANDLE_API_KEY \
  ghcr.io/weatherpal-ai/mailchimp-mcp-server:latest
...
**OAuth Setup:** Mailchimp requires OAuth authentication. Use `MCP_HANDLE_API_KEY` from your [free API key](https://www.mcp-handle.dev/home/api-keys) to handle the OAuth flow automatically.
