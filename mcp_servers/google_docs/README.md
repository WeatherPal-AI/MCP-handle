...
server = mcp_handle.mcp_server.create_server_instance("GOOGLE_DOCS", "user123")
```
...
```bash
# Pull latest image
docker pull ghcr.io/weatherpal-ai/google-docs-mcp-server:latest


# Run Google Docs MCP Server with OAuth Support through MCP Handle AI
docker run -p 5000:5000 -e MCP_HANDLE_API_KEY=$MCP_HANDLE_API_KEY \
  ghcr.io/weatherpal-ai/google-docs-mcp-server:latest
...
**OAuth Setup:** Google Docs requires OAuth authentication. Use `MCP_HANDLE_API_KEY` from your [free API key](https://www.mcp-handle.dev/home/api-keys) to handle the OAuth flow automatically.
