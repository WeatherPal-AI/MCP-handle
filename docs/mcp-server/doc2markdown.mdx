      ...
      <Step title="Create a Strata MCP Server with Doc2markdown">
        <CodeGroup>
          ...
          const response = await mcp_handle.mcpServer.createStrataServer({
              servers: [MCP Handle.McpServerName.Doc2markdown],
              userId: "user123"
          });
          ```
  ...
  <Tab title="Open Source">
    <Steps>
      <Step title="Clone repository (for developers to build from source)">
        ...
        cd mcp_handle/mcp_servers/doc2markdown
        ```
      </Step>
      ...
      <Step title="Run with Docker">
        ...
        docker pull ghcr.io/weatherpal-ai/doc2markdown-mcp-server:latest
        
        # Run with API key
        docker run -p 5000:5000 \
          -e AUTH_DATA='{"api_key":"your_doc2markdown_api_key"}' \
          ghcr.io/weatherpal-ai/doc2markdown-mcp-server:latest
        ```
      </Step>
...
<Tip>
With our progressive discovery approach, MCP Handle System is capable of enabling all tools for Doc2markdown. Please use the [get_tools](https://www.mcp-handle.dev/docs/api-reference/mcp-server/get-tools) API for more details. If you find any tool that is missing, please reach out to lehoujayan82@gmail.com.
</Tip>
