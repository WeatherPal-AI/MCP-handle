    ...
    <Steps>
      ...
      <Step title="Create a Strata MCP Server with Airtable">
        <CodeGroup>
          ...
          const response = await mcp_handle.mcpServer.createStrataServer({
              servers: [MCP Handle.McpServerName.Airtable],
              userId: "user123"
          });
          ```
      ...
      <Step title="Authenticate">
        <CodeGroup>
          ...
          // Open OAuth authorization page
          await open(response.oauthUrls[MCP Handle.McpServerName.Airtable]);
          ```
          
          ```bash cURL
          # Copy and paste the OAuth URL into your browser
          echo "Visit this URL to authorize: https://api.mcp-handle.dev/oauth/airtable/authorize?instance_id=YOUR_INSTANCE_ID"
          ```
        </CodeGroup>
  ...
  <Tab title="Open Source">
    <Steps>
      <Step title="Clone repository (for developers to build from source)">
        ...
        cd mcp_handle/mcp_servers/airtable
        ```
      </Step>
...
<Tip>
With our progressive discovery approach, MCP Handle System is capable of enabling all tools for Airtable. Please use the [get_tools](https://www.mcp-handle.dev/docs/api-reference/mcp-server/get-tools) API for more details. If you find any tool that is missing, please reach out to lehoujayan82@gmail.com.
</Tip>
