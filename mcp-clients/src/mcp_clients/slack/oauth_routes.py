...
def setup_oauth_routes(router: APIRouter):
    ...
    async def install_slack_app(account_id: str = None):
        ...
        state = None
        # Create state parameter with account_id if provided, this will be passed when user add MCPHandle powered Slack MCP Client
        if account_id:
            logger.info(f"Installing MCPHandle powered Slack MCP Client with account_id: {account_id}")
...
def create_welcome_blocks(dashboard_url: str):
    return [
        ...
        },
        {
            ...
            "text": {
                "type": "mrkdwn",
                "text": "Connect to an MCP server to get started, then simply mention the MCP Handle bot in a channel or send a direct message."
            }
        },
        {
            "type": "actions",
